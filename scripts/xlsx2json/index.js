let xlsx = require('./lib/xlsx-to-json.js');
...
function exportJson(args) {
    if (typeof args === 'undefined' || args.length === 0) {
        glob(path.join(__dirname, config.xlsx.src), function (err, files) {
            if (err) {
                console.error("exportJson error:", err);
                throw err;
            }
            files.forEach(function (element, index, array) {
                xlsx.toJson(element, path.join(__dirname, config.xlsx.dest), config.xlsx);
            });
        ...
        if (args instanceof Array) {
            args.forEach(function (element, index, array) {
                xlsx.toJson(path.join(__dirname, element), path.join(__dirname, config.xlsx.dest), config.xlsx);
            });
        }
    }
}
